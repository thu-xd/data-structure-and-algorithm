...
//The red black tree must satisfy four conditions as follow:
//1. The _root is black.
//2. The nullptr is black.
//3. The children of red must be black.
//4. For every node, the all simple paths from the node to descendant
//   leaves(leaves means nullptr) contain the same number of black nodes.
#include<algorithm>
#include<iostream>
...
template<typename T>
Node<T> * Red_Black_Tree<T>::search(const T & e)
...
template<typename T>
Node<T> * Red_Black_Tree<T>::insert(const T & e)
  //insert new element e into the red black tree.
{
  Node<T> * pos=search(e);
  if(pos) return pos;
  if(!last_valid_node) return _root=new Node<T>(e);
  Node<T> * insert_node=new Node<T>(e,RB_RED,nullptr,nullptr,last_valid_node);
...
template<typename T>
void Red_Black_Tree<T>::solve_double_red(Node<T> * x)   //x must be red.
...
template <typename T>
void Red_Black_Tree<T>::erase(const T & e)
...
template <typename T>
void Red_Black_Tree<T>::solve_double_black(Node<T> * child, Node<T> * parent)
...
template <typename T>
void Red_Black_Tree<T>::right_rotation(Node<T> * x)
...
template<typename T>
void Red_Black_Tree<T>::left_rotation(Node<T> * x)
